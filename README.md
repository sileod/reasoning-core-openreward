...
- `RC_ALLOWED_MODES` (default `instruct,few_shot`; only applied when a dataset has a `mode` column)
- `RC_DISABLE_HF_FALLBACK=1` to fail startup if Hugging Face loading fails instead of using procedural fallback
...
- By default, tasks are loaded from Hugging Face dataset `reasoning-core/formal-reasoning-env`. If the dataset has no `test` split, test tasks are taken from the `train` split after the train slice.
